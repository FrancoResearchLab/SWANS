...
#table_choices <- c('_top100_markers_avg_log2FC_', '_z_scores.', '_clusterProportions.experiment_', '_phase_experiment_', '_phase_sample_')
table_choices <- c('_top100_markers_avg_log2FC_', '_z_scores.', '_clusterProportions.experiment_', '_phase_experiment_')

# Create image output dir if not exists
if (!dir.exists('compare_images')){
  dir.create('compare_images')
}
# -----------------------------------------
...
shinyApp(
  ui <- fluidPage(
    ...
    h2('Tables'),
	 ...
	 h5('Choose between looking at differentially expressed genes (DEGs (top100_markers_avg_log2FC)), z-scores, number of cells in each cluster (clusterProportions.experiment), or the frequency of each phase.'),
...
#  ),
	 ...
	 h6('_phase_experiment_: Number of cells in each phase per cluster, split by experimental condition. Columns: Experiment — Experimental condition; cluster — Cluster ID; Phase — G1, G2M, S; numCells - Number of cells in that phase'),
 
    fluidRow(
		column(3,  selectInput("tab1", label = "dge, z-scores, cluster counts, or phase counts for Plot 1:", choices=table_choices), numericInput('cluster1', 'Cluster number from plot 1', value = -1, min = 0, max = 50)),
		column(3, offset = 1, selectInput("tab2", label = "dge, z-scores, cluster counts, or phase counts for Plot 2:", choices=table_choices), numericInput('cluster2', 'Cluster number from plot 2', value = -1, min = 0, max = 50)),
		column(3, offset = 1, selectInput("tab3", label = "dge, z-scores, cluster counts, or phase counts for Plot 3:", choices=table_choices), numericInput('cluster3', 'Cluster number from plot 3', value = -1, min = 0, max = 50))
    ),
...
fluidRow(
  ...
  server <- function(input, output) {
   ...
   phase_col_name_o <- c('Experiment', 'Cluster', 'Phase', 'Frequency')
   phase_col_name <- c('experiment', 'cluster', 'phase', 'numCells')
    ...
    output$top100_bycluster_tbl1 <- renderDataTable({
      ...
      } else if (input$tab1 == '_phase_experiment_') {
        disp_top100tbl_1 <- top100tbl_1[,phase_col_name_o]
        disp_top100tbl_1 <- disp_top100tbl_1[order(disp_top100tbl_1$Cluster, disp_top100tbl_1$Phase), ]
        colnames(disp_top100tbl_1) <- phase_col_name
    ...
    output$top100_bycluster_tbl2 <- renderDataTable({
      ...
      } else if (input$tab2 == '_phase_experiment_') {
        disp_top100tbl_2 <- top100tbl_2[,phase_col_name_o]
        disp_top100tbl_2 <- disp_top100tbl_2[order(disp_top100tbl_2$Cluster, disp_top100tbl_2$Phase), ]
        colnames(disp_top100tbl_2) <- phase_col_name
    ...
    output$top100_bycluster_tbl3 <- renderDataTable({
      ...
      } else if (input$tab3 == '_phase_experiment_') {
        disp_top100tbl_3 <- top100tbl_3[,phase_col_name_o]
        disp_top100tbl_3 <- disp_top100tbl_3[order(disp_top100tbl_3$Cluster, disp_top100tbl_3$Phase), ]
        colnames(disp_top100tbl_3) <- phase_col_name
