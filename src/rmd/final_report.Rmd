...
params:
  ...
  prelim_qc_config: ../../configs/prelim_configs.yaml
  post_annotation_config: ../../configs/post_annotation_configs.yaml
showWarnings: FALSE
...
# Allow direct reference to all variables in params
attach(params)

htmltools::htmlDependency("jquery", "3.6.0", src = "https://code.jquery.com/", script = "jquery-3.6.0.min.js")

# Sample Metadata
meta <- read.csv(file.path(sample_list), sep = '\t', header = TRUE)
meta <- meta[, 1:2]
samples <- meta$samples


# Configs
proj_info <- read_yaml(prelim_qc_config)
...
thresholds_used_file <- file.path(data_dir, project, 'analysis', 'report', 'tables', paste0(project, '_thresholds_used.tsv'))
thresholds_used <- read.table(thresholds_used_file, sep = '\t', header = TRUE)
...
final_info <- read_yaml(post_annotation_config)
...
cluster_annotation <- file.path(root_dir, final_info$`CLUSTER_ANNOTATION_FILE`)
file_user_gene_file <- file.path(root_dir, final_info$`FINAL_USER_GENE_FILE`) #need to double check if this causes an error
...
# Sig PC 
sig_pc_file <- file.path(data_dir, project, 'analysis', 'tables', paste0(project, '_', normalization, '_pca_sigPC.txt'))
...
#-------------------------------------------------------------------------------
final_markers_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_final_markers_total.txt'))
...
csv_folder <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', 'csv_files')
...
# adjusted p.value
final_pathways_file.adjP <- file.path(data_dir, project, 'analysis', 'final_analysis', 'pathway_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_final_filtered_results_combined_pathways_p_val_adj.csv'))
final_pathways_adjP <- ''
...
# avglog2FC
final_pathways_file.log <- file.path(data_dir, project, 'analysis', 'final_analysis', 'pathway_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_final_filtered_results_combined_pathways_avg_log2fc.csv'))
final_pathways_log <- ''
...
#-------------------------------------------------------------------------------
counts_sample_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_final_cluster_counts_proportions_sample.txt'))
counts_sample <- read.table(counts_sample_file, header=TRUE, sep='\t')

counts_experiment_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_final_cluster_counts_proportions_experiment.txt'))
counts_experiment <- read.table(counts_experiment_file, header=TRUE, sep='\t')
#-------------------------------------------------------------------------------

# Cell Cycling Phase + Frequency
#-------------------------------------------------------------------------------
phase_table_experiment_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_phase_experiment.txt'))
phase_table_experiment <- read.table(phase_table_experiment_file, header=TRUE, sep='\t')

phase_table_sample_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_phase_sample.txt'))
phase_table_sample <- read.table(phase_table_sample_file, header=TRUE, sep='\t')
#-------------------------------------------------------------------------------

# Z-scores
#-------------------------------------------------------------------------------
z_scores_file_celltypes <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_z_scores.', normalization, '.', integration, '_snn_res.', resolution, '_celltypes.txt'))
...
z_scores_file_celltypes_experiment <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_z_scores.', normalization, '.', integration, '_snn_res.', resolution, '_celltypes_experiment.txt'))
...
z_scores_file_celltypes_experiment_samples <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_z_scores.', normalization, '.', integration, '_snn_res.', resolution, '_celltypes_experiment_sample.txt'))
...
# dynamic z-scores
z_folder <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', 'z_score_files/')
...
#z_files <- file.path(paste0(data_dir, project, '/analysis/final_analysis/tables/', project, '_z_scores.', normalization, '.', integration, '_snn_res.', resolution, z_selection))
z_files <- file.path(data_dir, project, 'analysis', 'final_analysis', 'tables', paste0(project, '_z_scores.', normalization, '.', integration, '_snn_res.', resolution, z_selection))
...
{
  conserved_genes_file <- paste(data_dir, project, '/analysis/final_analysis/tables/conserved_genes/', project, '_', normalization, '.', integration, '_snn_res.', resolution, '_combined_conserved_markers.txt', sep='')
...
variable_genes_file <- file.path(data_dir, project, 'analysis', 'tables', paste0(project, '_', normalization, '_variable_genes.txt'))
...
# IMAGES
#-------------------------------------------------------------------------------
umap <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap.png'))
umap_phase <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap_phase.png'))
umap_phase_experiment <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap_phase_experiment.png'))
umap_phase_sample <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap_phase_sample.png'))
phase_experiment <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_geom_point_phase_sample.png'))
phase_sample <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_geom_point_phase_experiment.png'))
umap_experiment <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap_experiment.png'))
umap_sample <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '.', integration, '_snn_res.', resolution, '_umap_sample.png'))
proportions_sample <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_barplot_proportions_sample_', normalization, '.', integration, '_snn_res.', resolution, '.png'))
proportions_experiment <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_barplot_proportions_experiment_', normalization, '.', integration, '_snn_res.', resolution, '.png'))
heatmap_plot <- file.path(data_dir, project, 'analysis', 'final_analysis', 'figures', paste0(project, '_', normalization, '_variable_genes_heatmap.png'))
trajectory_clusters_file <- file.path(data_dir, project, 'analysis', 'final_analysis', 'trajectory_analysis', 'figures', paste0(project, '_monocle3_clusters.png'))
#-------------------------------------------------------------------------------
```
...
```{r echo=FALSE, include=TRUE}
datatable(data = thresholds_used, caption = 'Table 1a: Filtering Thresholds.', options=list(pageLength=10))
```
...
### Cell Cycling Phase UMAP (split by sample + experiment)

<div class = "row">
<div class = "column" style="flex: 6; padding-right: 10px;">
```{r  umapp_se, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='UMAP (sample)'}
	knitr::include_graphics(umap_phase_sample)
```
</div>

<div class = "column" style="flex: 6; padding-right: 1px;">
```{r umapp_s, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='UMAP (Cell Cycle Phase)'}
	knitr::include_graphics(umap_phase_experiment)
```
</div>
</div>
<br>

#### Cell Cycle Phase + Frequency by Sample
<div class = "row">
<br>
<div class = "column" style="flex: 6; padding-right: 10px;">
```{r psg, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='Cell Cycle Phase + Frequency by Sample)'}
	knitr::include_graphics(phase_sample)
```
</div>

<div class = "column" style="flex: 4;">
```{r pst, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='Cell Cycle Phase + Frequency by Sample'}
	datatable(data = phase_table_sample, caption = 'Table 8: Number of cells by phase, cluster, and sample.', filter='none', options=list(pageLength=10, autoWidth = TRUE, dom = 'Blfrtip', buttons = c('colvis', 'copy', 'excel', 'print')))
```
</div>
</div>
<br>

#### Cell Cycle Phase + Frequency by Experiment
<div class = "row">
<br>
<div class = "column" style="flex: 6; padding-right: 10px;">
```{r peg, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='Cell Cycle Phase + Frequency by Experiment'}
	knitr::include_graphics(phase_experiment)
```
</div>

<div class = "column" style="flex: 6; padding-right: 1px;">
```{r pet, include=T, echo=F, out.width="100%", out.height="100%", fig.cap='Cell Cycle Phase + Frequency by Experiment'}
	datatable(data = phase_table_experiment, caption = 'Table 9: Number of cells by phase, cluster, and experiment', filter='none', options=list(pageLength=10, autoWidth = TRUE, dom = 'Blfrtip', buttons = c('colvis', 'copy', 'excel', 'print')))
```
</div>
</div>
...
<br>
```{r echo=FALSE, include=TRUE}
datatable(data = variable_genes, rownames = FALSE, caption = 'Table 10: Variable Genes.', filter='none', options=list(pageLength=10, autoWidth = TRUE, dom = 'Blfrtip', buttons = c('copy', 'excel', 'print')))
```
</div>
</div>
...
if (conserved_genes == 'y' || conserved_genes == TRUE)
{
  datatable(data=conserved_genes_df, class = 'cell-border stripe', caption = 'Table 11: Conserved Genes by Across Exerimental Conditions by Cluster.', rownames = FALSE, filter = 'bottom', extensions = 'Buttons', width = '100%', options = list(autoWidth = TRUE, dom = 'Blfrtip', buttons = c('colvis', 'copy', 'excel', 'pdf', 'print')))
} else {
