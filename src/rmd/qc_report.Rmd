...
params:
  ...
  qc_config: ../../configs/prelim_configs.yaml
showWarnings: FALSE
...
## Filtering thresholds are per-sample now (set during the QC metrics review
## stage, either accepted as suggested or hand-edited by the user), so they
## no longer live as single global values in prelim_configs.yaml. The
## pipeline copies whichever thresholds file was actually used into the
## report folder as a record — read that instead.
...
if (run_doubletFinder == TRUE) {
  ...
  for (sample in samples) {
    ...
    sample_dir = file.path(data_dir, project, sample, 'doubletFinder')

    # Path to figure
    doublet_imgs[[sample]] = file.path(sample_dir, 'figures', paste0(project, '_', sample, '_doublet.png'))

    # Data
    stats = read.table(
        file = file.path(sample_dir, 'tables', paste0(project, '_', sample, '_doubletfinder_stats.csv')),
        sep = ',',
        header = TRUE
    )

    doublet_count = read.table(
        file = file.path(sample_dir, 'tables', paste0(project, '_', sample, '_doublet_ids.txt')),
        sep = ',',
        header = TRUE
...
if (run_soupX == TRUE) 
{
  for (sample in samples) 
  {
    sample_soupX_dir = file.path(data_dir, project, sample,'soupX')

    # Path to figure
    soupX_imgs[[sample]] = file.path(sample_soupX_dir, paste0(project, '_', sample, '_contam_plot.png'))
  }
}
...
analysis_dir = file.path(data_dir, project, '/analysis/figures')

# Pre-filt images (qc_1)
prefilt_vln = file.path(analysis_dir, paste0(project, '_', 'qc_1_vln.png'))
prefilt_scat1 = file.path(analysis_dir, paste0(project, '_', 'qc_1_scatter1.png'))
prefilt_scat2 = file.path(analysis_dir, paste0(project, '_', 'qc_1_scatter2.png'))

# Post-filt images (qc_2)
postfilt_vln = file.path(analysis_dir, paste0(project, '_', 'qc_2_vln.png'))
postfilt_scat1 = file.path(analysis_dir, paste0(project, '_', 'qc_2_scatter1.png'))
postfilt_scat2 = file.path(analysis_dir, paste0(project, '_', 'qc_2_scatter2.png'))
...
Filtering thresholds used (per sample, set during the QC metrics review stage):
```{r thresholds-used-table}
datatable(thresholds_used, options = list(pageLength = 10), rownames = FALSE)
```
