...
# Read in configs
proj_info <- read_yaml(params$qc_config)
## Main project info
...
if (run_doubletFinder == TRUE) {
  ...
  for (sample in samples) {
    ...
    sample_dir = file.path(params$data_dir, project, sample, 'doubletFinder/')
    ...
    # Data
    stats = read.table(
        file = file.path(sample_dir, 'tables/', project, '_', sample, '_doubletfinder_stats.csv'),
        sep = ',',
        header = TRUE
    )

    doublet_count = read.table(
        file = file.path(sample_dir, 'tables', project, '_', sample, '_doublet_ids.txt'),
        sep = ',',
        header = TRUE
...
if (run_soupX == TRUE) 
{
  for (sample in samples) 
  {
    sample_soupX_dir = file.path(params$data_dir, project, sample,'soupX')
...
analysis_dir = file.path(params$data_dir, project, '/analysis/figures')
